...
CharacterVector raggedToWide(const VecVecStr& ragged, int max_per_pt, const VecStr &visitIds) {
...
//' @export
// [[Rcpp::export]]
CharacterVector icd9LongToWideCpp(const SEXP& icd9df, const std::string visitId="visitId",
	...
	UNPROTECT(1);
	if (TYPEOF(vsexp)!=STRSXP)
		stop("visitIds should be pre-converted to str - which is necessary for matrix rowname output anyway");
