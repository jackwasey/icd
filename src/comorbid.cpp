...
// [[Rcpp::export]]
Rcpp::LogicalVector icd9ComorbidShortCpp(const SEXP& icd9df, const Rcpp::List& icd9Mapping,
  ...
  DEBUG(vcdb.size() << " visits, " << num_comorbid << " is num_comorbid");
  ...
  DEBUG("converted from ComorbidOut to vec bool, so Rcpp can handle cast to R logical vector");
  ...
  DEBUG("Ready to transpose and return");
