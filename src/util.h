...
template <class COCiter, class Oiter> void my_concat(COCiter start, COCiter end, Oiter dest) {
  while (start != end) {
...
getDataFrameStringRow(const Rcpp::DataFrame& df, const R_xlen_t i, const R_xlen_t start_idx = 0) {
