...
test_that("extract decimal parts - valid inputs", {
  expect_equal(icd_decimal_to_parts.icd9("0"), list(mjr = "000", mnr = ""))
  expect_equal(icd_decimal_to_parts.icd9("000"), list(mjr = "000", mnr = ""))
  expect_equal(icd_decimal_to_parts.icd9("V1.2"), list(mjr = "V01", mnr = "2"))
  expect_equal(icd_decimal_to_parts.icd9("1.1"), list(mjr = "001", mnr = "1"))
  expect_equal(icd_decimal_to_parts.icd9("001.1"), list(mjr = "001", mnr = "1"))
  expect_equal(icd_decimal_to_parts.icd9("22.22"), list(mjr = "022", mnr = "22"))
  expect_equal(icd_decimal_to_parts.icd9("022.22"), list(mjr = "022", mnr = "22"))
  expect_equal(icd_decimal_to_parts.icd9("333.3"), list(mjr = "333", mnr = "3"))
  expect_equal(icd_decimal_to_parts.icd9("444"), list(mjr = "444", mnr = ""))
  expect_equal(icd_decimal_to_parts.icd9("444", mnr_empty = NA_character_),
               list(mjr = "444", mnr = NA_character_))
  expect_equal(icd_decimal_to_parts.icd9("444", mnr_empty = ""),
               list(mjr = "444", mnr = ""))
  ...
  expect_equal(icd_decimal_to_parts.icd9(c("9.9", "88", "777.6"), mnr_empty = NA_character_),
               list(
                 mjr = c("009", "088", "777"),
  ...
  expect_equal(icd_decimal_to_parts.icd9(c("9.9", "88", "777.6"), mnr_empty = ""),
               list(
                 mjr = c("009", "088", "777"),
                 mnr = c("9", "", "6")
               )
  ...
  expect_equal(icd_decimal_to_parts.icd9(c("001", "", "991.23"), mnr_empty = NA_character_),
               list(
...
test_that("icd10 short to decimal", {
  expect_true(is.icd_decimal_diag(short_to_decimal("A00")))
  expect_true(is.icd_decimal_diag(short_to_decimal.icd10("A00")))
  expect_true(is.icd_decimal_diag(short_to_decimal.icd10cm("A00")))
  expect_true(is.icd_decimal_diag(short_to_decimal("A009")))
  expect_true(is.icd_decimal_diag(short_to_decimal.icd10("A009")))
  expect_true(is.icd_decimal_diag(short_to_decimal.icd10cm("A009")))

  expect_equal(short_to_decimal("A00"), as.icd10("A00") %>% as.icd_decimal_diag)
  expect_equal(short_to_decimal.icd10("A00"), as.icd10("A00") %>% as.icd_decimal_diag)
  expect_equal(short_to_decimal("A000"), as.icd10("A00.0") %>% as.icd_decimal_diag)
  expect_equal(short_to_decimal.icd10("A000"), as.icd10("A00.0") %>% as.icd_decimal_diag)

})

test_that("#97 is fixed", {
  expect_error(res <- short_to_decimal(icd10_map_elix$CHF), NA)
...
test_that("icd10 short to decimal for multiple codes", {
  ...
  expect_error(res <- short_to_decimal(c("O9A119", "O9A53", "S0000XA", "T3299", "P150", "P159",
  ...
  expect_error(res <- short_to_decimal(icd10cm(c("O9A119", "O9A53", "S0000XA", "T3299", "P150", "P159",
                                                       "Z9989", "Z950", "C7A098", "C7A8"))), regexp = NA)
...
test_that("icd10 short to decimal and back", {
  expect_identical(short_to_decimal(icd_decimal_to_short("A00.0")), as.icd10("A00.0") %>% as.icd_decimal_diag)
  expect_identical(icd_decimal_to_short(short_to_decimal("A000")), as.icd10("A000") %>% as.icd_short_diag)
  expect_identical(short_to_decimal.icd10(icd_decimal_to_short.icd10("A00.0")), as.icd10("A00.0") %>% as.icd_decimal_diag)
  expect_identical(icd_decimal_to_short.icd10(short_to_decimal.icd10("A000")), as.icd10("A000") %>% as.icd_short_diag)
})
...
test_that("running short to decimal conversion before and after expansion
          of a ICD-9 base codes gives the same result", {
            ...
            for (i in names(icd9List)) {
              expect_equal_no_icd(
                icd_decimal_to_short.icd9(short_to_decimal.icd9(icd9List[[i]])),
                icd9_add_leading_zeroes(icd9List[[i]], short_code = TRUE),
                info = paste("step 1, iteration:", i)
              )

              expect_equal_no_icd(
                icd_decimal_to_short(short_to_decimal(icd9List[[i]])),
                icd9_add_leading_zeroes(icd9List[[i]], short_code = TRUE),
            ...
            expect_equal_no_icd(
              short_to_decimal.icd9(icd_decimal_to_short.icd9(rd9)),
              rd9pad
            )

            # same with function to dispatch appropriately
            expect_equal_no_icd(
              short_to_decimal(icd_decimal_to_short(rd9)),
              rd9pad
            )
            ...
            expect_equal_no_icd(short_to_decimal.icd9(icd_decimal_to_short.icd9(rd2)), rd2pad)
            expect_equal_no_icd(icd_decimal_to_short.icd9(short_to_decimal.icd9(rd2)), rd2pad)

            expect_equal_no_icd(short_to_decimal.icd9(icd_decimal_to_short.icd9(rd2pad)), rd2pad)
...
test_that("minimal test case for showing why padding zeroes before decimal conv without class matters", {
  expect_equal_no_icd(short_to_decimal(icd_decimal_to_short("12.3")), "012.3")
})
...
test_that("convert icd-9 ranges", {
  ...
  expect_error(test_map <- icd9_chapters_to_map(icd9_chapters), regexp = NA)
  expect_error(
...
test_that("code routes through RcppExports.R and slower versions", {
  ...
  expect_equal(short_to_parts.icd9(c("99999", "0011")),
               data.frame(mjr = c("999", "001"), mnr = c("99", "1"), stringsAsFactors = FALSE))
})
