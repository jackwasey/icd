...
test_that("constructing a comorbidity map with unnamed list, etc. fails", {
  ...
  expect_error(as.icd_comorbidity_map(vermont_dx))
  expect_error(as.icd_comorbidity_map(uranium_pathology))
})
...
test_that("subsetting data frame works", {
  expect_equal(unclass(vermont_dx[1, 6]), as.icd_short_diag("27801"))
  expect_equal(unclass(vermont_dx[1, "DX1"]), as.icd_short_diag("27801"))
