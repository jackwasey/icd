...
expect_icd10cm_child_is_self <- function(...) {
  ...
  for (i in dots) {
    ...
    eval(bquote(expect_no_warn(warn_res <- icd_children(as.icd10(.(i))))))
    ...
    eval(bquote(expect_identical(warn_res,
                                 as.icd10(as.icd_short_diag(.(i))))))

    eval(bquote(expect_identical(icd_children.icd10cm(.(i)),
                                 as.icd10cm(as.icd_short_diag(.(i))))))
    eval(bquote(expect_identical(icd_children.icd10cm(as.icd10(.(i))),
                                 as.icd10cm(as.icd_short_diag(.(i))))))
    eval(bquote(expect_identical(icd_children.icd10cm(as.icd10cm(.(i))),
                                 as.icd10cm(as.icd_short_diag(.(i))))))

    # the children are only even icd10cm, but we should not enforce this:
...
test_that("zero length ICD-10-CM children", {
  expect_empty_icd10cm_kids <- function(x, has_warning = TRUE) {
    # just for deprecation, suppress warnings:
    suppressWarnings(
      res <- icd_children_defined.icd10cm(as.icd10cm(x))
    )
  ...
  expect_no_warn(
    icd_children_defined(icd10cm(character(0))), icd10cm(character(0)))
})
