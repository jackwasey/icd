...
test_that("if an ICD class is not specified for a data set, but is for a column therein,
          the correct method is dispatched", {
            ...
            expect_error(icd9_comorbid_quan_elix(mydf), regexp = NA)
            expect_error(icd9_comorbid_quan_deyo(mydf), regexp = NA)
            expect_error(icd9_comorbid_elix(mydf), regexp = NA)
            expect_error(icd9_comorbid_ahrq(mydf), regexp = NA)
          })
