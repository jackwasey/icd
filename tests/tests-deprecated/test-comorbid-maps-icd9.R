...
test_that("condense big lists of comorbidities", {
  ...
  # reverse)
  ahrq <- lapply(icd9_map_ahrq[c(1, 30)],
                 icd_condense.icd9, short_code = TRUE, defined = FALSE)
  qd <- lapply(icd9_map_quan_deyo[c(1, 17)],
               icd_condense.icd9, short_code = TRUE, defined = FALSE)
  qe <- lapply(icd9_map_quan_elix[c(1, 30)],
               icd_condense.icd9, short_code = TRUE, defined = FALSE)
  elix <- lapply(icd9_map_elix[c(1, 30)],
                 icd_condense.icd9, short_code = TRUE, defined = FALSE)
...
test_that("failing example", {
  ...
  expect_is(icd_charlson(mydf, short_code = FALSE, return_df = TRUE),
            "data.frame")
