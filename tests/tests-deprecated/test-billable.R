...
test_that("ICD-9-CM billable codes package data is recreated", {
  ...
  # Do encoding problems on Linux. It is unpredictable at the best of times. We
  # can do this offline if we have all (currently available) versions of the
  ...
  expect_data_frame(b32, nrows = 14567L, any.missing = FALSE,
                    ncols = 3L, types = "character")
  ...
  for (ver in c("27", "28", "29", "30", "31", "32")) {
    ...
    diff <- v != cb
    expect_identical(
      check_billable[[ver]], icd9cm_billable[[ver]],
