...
# at present, icd9cm_hierarchy is derived from RTF parsing, a little web
...
test_that("factors are in the right place", {
  expect_is(icd9cm_hierarchy[["code"]], c("icd9cm", "icd9", "character"))
...
test_that("codes and descriptions are valid and unique", {
  ...
  expect_true(all(icd_is_valid.icd9(icd9cm_hierarchy[["code"]])))
})

test_that("some chapters are correct", {
  chaps <- jwutil::as_char_no_warn(icd9cm_hierarchy$chapter)
  ...
  expect_equal(chaps[nrow(icd9cm_hierarchy)],
...
test_that("some sub-chapters are correct", {
  subchaps <- jwutil::as_char_no_warn(icd9cm_hierarchy$sub_chapter)
  codes <- icd9cm_hierarchy[["code"]]

  # first and last
  expect_equal(subchaps[1], "Intestinal Infectious Diseases")
  expect_equal(subchaps[nrow(icd9cm_hierarchy)], "Injury Resulting From Operations Of War")
