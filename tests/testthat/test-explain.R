...
test_that("github issue #41", {
  expect_equal(
    icd_explain.icd9(icd_get_defined.icd9(quanDeyoComorbid[["Dementia"]]), condense = TRUE),
    icd_explain.icd9(quanDeyoComorbid[["Dementia"]], condense = TRUE, warn = FALSE)
  )
  expect_equal(
    icd_explain.icd9(icd_get_defined(quanDeyoComorbid[["Dementia"]]), condense = FALSE),
...
test_that("parse icd9_majors vs those listed
          in the other CDC source of the leaf definitions.", {
            ...
            compareMajors <- unique(icd_get_major.icd9(icd9::icd9cm_hierarchy[["code"]], short_code = TRUE))
...
test_that("unsorted hierarchy tests", {
  expect_equal(
    tolower(icd9::icd9cm_hierarchy[icd9cm_hierarchy[["code"]] == "00321",
                                "descLong"]),
