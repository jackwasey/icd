...
test_that("long data to wide data", {
  longcmp <- data.frame(visit_id = c("a", "b", "c"),
                        ...
                        icd_002 = c(NA, "443", NA))
  expect_equivalent(res <- long_to_wide(longdf, return_df = TRUE), longcmp)
  expect_true(is.icd_wide_data(res))
  longcmp2 <- data.frame(visit_id = c("a", "b", "c"),
                         icd_001 = c("441", "4424", "441"),
                         icd_002 = c(NA, "443", NA),
                         icd_003 = c(NA, NA, NA))
  expect_equivalent(res <- long_to_wide(longdf, min_width = 3, return_df = TRUE), longcmp2)
  expect_true(is.icd_wide_data(res))
  longdf2 <- data.frame(i = c("441", "4424", "443", "441"),
                        v = c("a", "b", "b", "c"))
  expect_equal(names(long_to_wide(longdf2,
                                  visit_name = "v",
                                  icd_name = "i",
...
test_that("dataframe to matrix and back", {
  for (pts in list(random_test_patients, test_twenty, multi_comorbid)) {
    ...
    df3 <- comorbid_mat_to_df(mat2, visit_name = "visit_id", stringsAsFactors = FALSE)
