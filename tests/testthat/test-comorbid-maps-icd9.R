...
test_that("ahrq icd9 map recreated", {
  ...
  expect_equivalent(icd_get_invalid.icd_comorbidity_map(icd9_map_ahrq), list())
...
test_that("condense big lists of comorbidities", {
  ...
  for (defined_codes in c(TRUE, FALSE)) {
    if (defined_codes) {
      expect_warning(ahrq <- lapply(icd9_map_ahrq[c(1, 30)], icd_condense.icd9, short_code = TRUE, defined = defined_codes))
      expect_warning(qd <- lapply(icd9_map_quan_deyo[c(1, 17)], icd_condense.icd9, short_code = TRUE, defined = defined_codes))
      expect_warning(qe <- lapply(icd9_map_quan_elix[c(1, 31)], icd_condense.icd9, short_code = TRUE, defined = defined_codes))
      expect_warning(elix <- lapply(icd9_map_elix[c(1, 31)], icd_condense.icd9, defined = defined_codes))
    }
    else {
      expect_warning(ahrq <- lapply(icd9_map_ahrq[c(1, 30)], icd_condense.icd9, short_code = TRUE, defined = defined_codes), regexp = NA)
      expect_warning(qd <- lapply(icd9_map_quan_deyo[c(1, 17)], icd_condense.icd9, short_code = TRUE, defined = defined_codes), regexp = NA)
      expect_warning(qe <- lapply(icd9_map_quan_elix[c(1, 30)], icd_condense.icd9, short_code = TRUE, defined = defined_codes), regexp = NA)
      expect_warning(elix <- lapply(icd9_map_elix[c(1, 30)], icd_condense.icd9, short_code = TRUE, defined = defined_codes), regexp = NA)
    }
...
test_that("icd9cm_hierarchy as saved in data can be recreated as expected", {
  ...
  cmh <- icd9cm_make_chapters_hierarchy(save_data = FALSE, verbose = FALSE, offline = TRUE)
...
test_that("built-in icd9 to comorbidity mappings are all valid", {
  expect_true(icd_is_valid.icd_comorbidity_map(icd9_map_ahrq, short_code = TRUE))
  expect_true(icd_is_valid.icd_comorbidity_map(icd9_map_quan_elix, short_code = TRUE))
  expect_true(icd_is_valid.icd_comorbidity_map(icd9_map_quan_elix, short_code = TRUE))
  expect_true(icd_is_valid.icd_comorbidity_map(icd9_map_elix, short_code = TRUE))
})

test_that("disordered visit ids", {
  pts <- data.frame(visit_id = c("2", "1", "2", "3", "3"),
                    icd9 = c("39891", "40110", "09322", "41514", "39891"))
  icd9_comorbid(pts, icd9_map_ahrq, short_code = TRUE)
})
...
test_that("diff comorbid works", {
  ...
  expect_warning(
    utils::capture.output(
      res <- icd_diff_comorbid(icd9_map_ahrq, icd9_map_elix, show = FALSE)
    ),
    regexp = NA)
