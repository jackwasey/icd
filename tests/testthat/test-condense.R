...
test_that("mix of four and five digit billable codes", {
  ...
  expect_equal(
    icd_condense.icd9(short = TRUE,c("1000", "10081", "10089", "1009")),
    "100")
})
...
test_that("mix of four and five digit billable codes over bigger range", {
  # this is all of leptospirosis, but missing the "1008" non-billable sub-heading
  expect_equal(
    icd_condense.icd9(short = TRUE,c("1000", "10081", "10089", "1009", "101")),
    c("100", "101"))
})
...
test_that("mix of four and five digit with non-billable mid-level four digit code", {
  expect_equal(
    icd_condense.icd9(short = TRUE,c("1000", "1008", "10081", "10089", "1009")),
    "100")
})
...
test_that("condense short range", {
  ...
  expect_that(res <- icd_condense.icd9(short = TRUE,othersalmonella, real = FALSE), testthat::not(gives_warning()))
  expect_equal(res, othersalmonella)
  # missing this leaf node, we can't condense at all
  expect_equal(icd_condense.icd9(short = TRUE,othersalmonella[-3], real = TRUE),
               othersalmonella[-3])
  # if we demand condensing to all possible values, we get the same back
  expect_equal(icd_condense.icd9(short = TRUE,othersalmonella[-3], real = FALSE),
               othersalmonella[-3])

  expect_equal(sort(icd9ChildrenShort(icd9Short = "001", onlyBillable = TRUE)),
               c("0010", "0011", "0019"))

  expect_equal(sort(icd9ChildrenShort(icd9Short = "001", real = TRUE)),
               c("001", "0010", "0011", "0019"))

  expect_equal(icd_condense.icd9(short = TRUE,icd9ChildrenShort("00320", real = TRUE), real = TRUE), "00320")
  # majors should be okay, even if not 'real'
  expect_that(dup_res <- icd_condense.icd9(short = TRUE,icd9ChildrenShort("003", real = TRUE)),
              testthat::not(gives_warning()))

  expect_equal(icd_condense.icd9(short = TRUE,c("003", "003"), real = TRUE), "003")
  expect_equal(icd_condense.icd9(short = TRUE,c("003", "003"), real = FALSE), "003")
})
...
test_that("condense full ranges", {
  ...
  # themselves defined.
  # majors:
  expect_equal(icd_condense.icd9(short = TRUE,icd9ChildrenShort("003", real = FALSE), real = FALSE), "003")
  expect_equal(icd_condense.icd9(short = TRUE,icd9ChildrenShort("3", real = FALSE), real = FALSE), "003")
  expect_equal(icd_condense.icd9(short = TRUE,icd9ChildrenShort("410", real = FALSE), real = FALSE), "410")
  expect_equal(icd_condense.icd9(short = TRUE,icd9ChildrenShort("V12", real = FALSE), real = FALSE), "V12")
  expect_equal(icd_condense.icd9(short = TRUE,icd9ChildrenShort("E800", real = FALSE), real = FALSE), "E800")
  # repeat some tests with decimals instead
  expect_equal(icd_condense.icd9Decimal(icd9Children("003", isShort = FALSE, real = FALSE), real = FALSE), "003")
  expect_equal(icd_condense.icd9(icd9ChildrenDecimal("3", real = FALSE), isShort = FALSE, real = FALSE), "003")
  ...
  expect_equal(icd_condense.icd9Decimal(icd9Children("003.2", isShort = FALSE, real = FALSE), real = FALSE),
               "003.2")
  expect_equal(icd_condense.icd9(icd9ChildrenDecimal("3.2", real = FALSE), isShort = FALSE, real = FALSE),
               "003.2")
...
test_that("condense a factor of codes instead of character vector", {
  ...
  dat <- as.factor(vdat)
  expect_equal(dat, icd_condense.icd9(short = TRUE,dat))

})

test_that("levels are preserved from source factor", {
  dat <- factor(vdat, levels = c("plastic", vdat))
  expect_identical(dat, icd_condense.icd9(short = TRUE,dat, keepFactorLevels = TRUE))
  expect_equivalent(dat, icd_condense.icd9(short = TRUE,dat, keepFactorLevels = FALSE))
})
