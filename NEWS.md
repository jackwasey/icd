 ...
 * `plot_comorbid` uses simple base R graphics to quickly summarise comorbidities in a bar plot.
 * Let `icd` work better when not lodaded with `library`, so `icd::comorbid_ahrq` etc. should work with just `requireNamespace`
 * Drop dependency on magrittr, although it may be used nicely with icd functions
 ...
 * Simplify by using Rcpp for package registration
 * Simplify by falling back to base R for long to wide conversion
 * Better documentation, including more links between documentation, removed internal documentation from public view
 * code cleaning and linting, e.g. replaced sapply with vapply throughout
