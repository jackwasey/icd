...
icd9_filter_valid <- function(x, icd_name = get_icd_name(x), short_code = icd_guess_short(x[[icd_name]]), invert = FALSE) {
  ...
  assert_flag(invert)
  assert_data_frame(x, min.cols = 1, col.names = "named")
  x[icd_is_valid.icd9(jwutil::as_char_no_warn(.subset2(x, icd_name)), short_code = short_code) != invert, ]
}
...
icd10_filter_valid <- function(x, icd_name = get_icd_name(x),
  ...
  assert_flag(short_code)
  assert_flag(invert)
  x[icd_is_valid.icd10(jwutil::as_char_no_warn(.subset2(x, icd_name)), short_code = short_code) != invert, ]
