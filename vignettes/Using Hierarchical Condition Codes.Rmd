...
The function `icd_comorbid_hcc` takes as input a data frame in the "long" format (i.e. mutiple rows per patients). If the data has one row per patient, but multiple codes in different columns, this is the "wide" format. `icd_wide_to_long` could be used to convert before inputting into `icd_comorbid_hcc`. Doing it this way, instead of with tools like `dplyr` allows some guessing for which columns are codes, and which are identifiers, and is also written specifically to do this task efficiently, given the usual structure of hospital data. `icd_comorbid_hcc` also requires the ICD codes to be in "short" format. If your codes are in "decimal" format, you can easily convert them with the function `icd_decimal_to_short`.
