...
<<"comorbidUsingDefaultOpts",echo=TRUE,eval=FALSE,cache=TRUE>>=
...
<<arbitrarycomorbiditymapinchain>>=
myData %>%
  icd9FilterPoaYes() %>%
  icd9Comorbidities(
...
The user can provide any ICD-9 mapping they wish. Included in this package is a small data set called icd9CmChapters, which lists the ICD-9-CM (and indeed ICD-9) Chapters. These can easily be expanded out and used as a mapping
<<"arbitraryMapping>>=
icd9CmChapters[1:5]
@
...
<<eval=FALSE>>=
# expand range, and find all children of these top-level codes in one lapply:
myMap <- lapply(ahrqComorbid[c("Obesity", "Depression")], icd9ChildrenShort)
system.time(
  patientChapters <- icd9Comorbidities(
    icd9df = patientData, icd9Mapping = myMap, isShortMapping = TRUE, validateMapping = FALSE)
)
...
The inverse operation to find condensed list using any parent codes that fully describe the same set is work in progress.
