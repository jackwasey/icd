...
## font size options from the highlight package:
...
icd9ComorbiditiesAhrq(icd9df = patientData)[, 1:5]
# or
icd9ComorbiditiesQuanDeyo(patientData)[, 1:5]
@
or using magrittr to chain functions together:
<<getcomorbidities2, eval=TRUE,cache=TRUE, size="small">>=
patientData %>% icd9FilterPoaYes() %>% icd9ComorbiditiesAhrq(isShort = TRUE) %>% extract(1:5)
@
...
<<"exampleAhrqComorbid",size="small",cache=TRUE>>=
ahrqComorbid[c("OBESE", "DEPRESS")]
#this latter is obsolete since all children are now included in the packaged
#mappings. The inverse operation to find condensed list using any parent codes
#that fully describe the same set is WIP.
