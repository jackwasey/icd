...
<<>>=
cardiac <- unique(c(
  icd9CmDesc[grepl(pattern="(heart)|(cardiac)", x = icd9CmDesc[["descLong"]], ignore.case = TRUE), "icd9"],
  icd9CmDesc[grepl(pattern="(heart)|(cardiac)", x = icd9CmDesc[["descShort"]], ignore.case = TRUE), "icd9"]
  )
)
@
...
# generate a range with more than two hundred (possible) ICD-9 codes
