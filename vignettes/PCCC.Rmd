...
set.seed(1441)
nm <- "code"
i <- if (icd:::icd_data_ver_ok()) {
  getExportedValue(asNamespace("icd.data"), "icd10cm2018_pc")
} else {
  nm <- "pcs"
  getExportedValue(asNamespace("icd.data"), "icd10_pcs_2016")
}
dat <- data.frame(
  id = n + seq(n),
  icd_dx1 = sample(icd.data::icd10cm2016$code, n, replace = TRUE),
  icd_dx2 = sample(icd.data::icd10cm2016$code, n, replace = TRUE),
  icd_pcs1 = sample(i[[nm]], n, replace = TRUE),
  icd_pcs2 = sample(i[[nm]], n, replace = TRUE)
)
```
