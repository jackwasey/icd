...
one_pt <- data.frame(
  id = rep("patient1", length(diagnoses)),
  diagnoses
)
dif_pt <- data.frame(
  id = paste0("patient", seq_along(diagnoses)),
  diagnoses
)
my_map <- list(
  c01to17 = expand_range("C01", "C17"),
...
```{r matriximage, fig.width=6, echo = FALSE}
{
  ...
  axis(1L,
    at = seq(0L, 1L, length.out = length(my_map)),
    labels = names(my_map),
    lwd = 0L
  )
  axis(2L,
    at = seq(0L, 1L, length.out = nrow(dif_pt)),
    labels = dif_pt$id,
    lwd = 0L,
    las = 2L
  )
