...
```{r getcomorbidities}
icd_comorbid_ahrq(patients)[, 1:8]
```
or
```{r getcomorbidities1a}
icd_comorbid_quan_deyo(patients)[, 1:8]
```
and things work beautifully using [magrittr](https://cran.r-project.org/package=magrittr) `%>%` to chain functions together. This is not a dependency for this package, but is recommended because of the frequent need to chain together *icd* commands, and greater clarity.
```{r getcomorbidities2}
patients %>% icd_filter_poa_yes %>% icd_comorbid_ahrq %>% extract(1:8)
```
...
```{r ahrq}
#ahrqComorbid <- icd:::sas_parse_ahrq() # user doesn't need to do this
names(icd9_map_ahrq)
...
system.time(
  icd_comorbid_ahrq(many_patients)
  )[["elapsed"]] 
```
...
The user can provide any ICD-9, ICD-10 or other code mapping to comorbidities they wish. Submissions of other peer-reviewed published mappings could be included in this package, if their license permits. Create an issue in [github](https://github.com/jackwasey/icd9/issues) or email me at jack@jackwasey.com) Included in this package is a small data set called `icd9_chapters`, which lists the ICD-9-CM (and indeed ICD-9) Chapters. These can easily be expanded out and used as a mapping, so instead of a comorbidity, we see which patients have codes in each chapter of the ICD-9 defintion.
```{r "arbitraryMapping"}
names(icd9_chapters)[c(1:5, 14)]
myMap <- icd:::icd9_chapters_to_map(icd9_chapters[c(2, 5, 14)])
icd_comorbid(patients, myMap) # no +ve 
```
