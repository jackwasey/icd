...
ICD-10 codes. Common ambiguities and code formats are supported. This
...
icd_comorbid_ahrq(patients_icd9)[, 1:8]
```
...
patients_icd9 %>% icd_filter_poa %>% icd_comorbid_elix %>% head
...
vermont_dx %>% icd_wide_to_long  %>% icd_comorbid_quan_deyo  %>% apply(2, as.integer) %>%  head
```

The above can be rewritten in classic R with many parentheses:
```{r lots of brackets, eval = FALSE}
head(apply(icd_comorbid_quan_deyo(icd_wide_to_long(vermont_dx)), 2, as.integer))
```
...
codes %>% as.icd_decimal_diag %>% icd10
```
...
df <- data.frame(i9 = icd9(c("100", "001")), 
                 i10 = icd10(c("Z771", "Z87820")))
...
test_code <- icd9(icd_short_diag("00321"))
...
icd_comorbid(patients_icd9, my_map) # no positive 
```
