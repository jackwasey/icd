...
```
Although `V10.0` would include ten possible children, the range with \code{onlyReal = FALSE} only returns 4 values to avoid the range returning broader classifications than intended (V10.0 would imply all its children).
