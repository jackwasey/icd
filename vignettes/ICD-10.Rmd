...
```{r uranium-long}
icd.data::uranium_pathology[1:10, ]
```
...
```{r uranium-wide}
head(icd.data::uranium_pathology)
# attach icd.data for convenience
library(icd.data)
```
...
nm = "code"
pcs <- if (icd:::icd_data_ver_ok()) {
  icd:::get_from_icd_data("icd10cm2018_pc", lazy = TRUE)
} else {
  nm <- "pcs"
  icd:::get_from_icd_data("icd10_pcs_2018", lazy = TRUE)
}
```
```{r icd10pcs}
set.seed(1441)
