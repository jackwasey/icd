...
suppressWarnings({
  suppressPackageStartupMessages({
    requireNamespace("knitr", warn.conflicts = FALSE) # for opts_chunk only
    library(icd9)
    library(magrittr)
    library(xtable)
    library(utils)
...
```{r uranium-wide}
uranium_pathology %>% icd9LongToWide %>% head
```

Now map these diagnoses to disease groups as defined by Quan et al:
```{r comorbidities}
quan_comorbidities <- icd9Comorbid(uranium_pathology, icd10_map_quan_elix)
