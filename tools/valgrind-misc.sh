...
R -d "valgrind --tool=callgrind --simulate-cache=yes" -e "library(icd9); icd:::icd9BenchComorbid(10000)"
# then run:
...
R -d "valgrind --tool=callgrind --simulate-cache=yes" -e "library(icd9); devnull<-icd:::sc(1000000)"
...
R -d "valgrind --tool=callgrind --simulate-cache=yes" -e "library(icd9); pts<-icd:::generate_random_pts(5e6,10); j<-icd9ComorbidShortOpenMPVecInt(pts, ahrqComorbid, threads = 1)"

R -d "valgrind --tool=callgrind --simulate-cache=yes" -e "library(icd9); library(microbenchmark); benchVaryn()"

# useful to install first...
R CMD INSTALL icd9 && R -d "valgrind --tool=callgrind"  -e "library(icd9); pts <- icd:::randomOrderedPatients(1000, 10); icd:::icd9LongToWideMatrix(pts)"
