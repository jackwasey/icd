...
R -d "$CALLGRIND" -e "library(icd); load('i9test.RData'); icd_comorbid_ahrq(i9test, visit_id = 'patcom', icd_name = 'i9diag')"
...
R CMD INSTALL ~/icd && Rscript $PKG_ARG -d "$CALLGRIND" -e "pts <- generate_random_ordered_pts(1000, 10); icd_long_to_wide(pts)"
