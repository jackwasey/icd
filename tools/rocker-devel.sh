...
docker run --rm -ti -v $(pwd):/mnt rocker/r-devel sh -c "apt-get update && apt-get install --yes --fix-missing littler libgit2-dev libxml2-dev/unstable libcurl4-gnutls-dev libssl-dev && /usr/share/doc/littler/examples/check.r --setwd /mnt --install-deps $1"
