...
test_that("ICD-10 comorbidities from uranium", {
  expect_error(icd_comorbid(uranium_pathology, icd10_map_quan_elix), regexp = NA)
  expect_error(icd_comorbid(uranium_pathology, icd10_map_quan_deyo), regexp = NA)
  expect_error(icd_comorbid(uranium_pathology, icd10_map_elix), regexp = NA)
  expect_error(icd_comorbid(uranium_pathology, icd10_map_ahrq), regexp = NA)
})
...
test_that("ahrq comorbidities found for test data", {
  ...
  for (test_name in names(td)) {
    ...
    expect_error(res <- icd_comorbid(td[[test_name]], map = icd10_map_ahrq), regexp = NA)
    for (n in colnames(res))
      expect_true(res[, n], info = paste("method one comorbidity:", n, ", test: ", test_name))

    expect_error(res <- icd10_comorbid(td[[test_name]], map = icd10_map_ahrq), regexp = NA)
    for (n in colnames(res))
...
test_that("ahrq comorbidities found for test data for multiple patients each with a different comorbidity", {
  ...
  for (test_name in names(td)) {
    ...
    res <- icd_comorbid(td[[test_name]], icd10_map_ahrq)
    for (n in colnames(res))
